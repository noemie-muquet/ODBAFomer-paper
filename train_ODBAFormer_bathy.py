...
import pandas as pd
from dataloaders.bathy_dataloader import ODBADataModule, ODBATestDataModule
    ...
    def save_vis_step_end(
        ...
        if self.local_rank == 0:
            ...
            if batch_idx in example_data_idx_list:
                ...
                im0 = ax[0,0].imshow(x, cmap = traj_cmap, vmin = 0)
                ax[0,0].set_title('Input Trajectory')
                fig.colorbar(im0, ax=ax[0,0])
                
                im1 = ax[1,0].imshow(xt, cmap = odba_cmap, vmin = 0)
                ax[1,0].set_title('Real ODBA')
                fig.colorbar(im1, ax=ax[1,0])
                
                im2 = ax[1,1].imshow(xp, cmap = odba_cmap, vmin = 0)
                ax[1,1].set_title('Predicted ODBA')
                fig.colorbar(im2, ax=ax[1,1])
                
                im3 = ax[0,1].imshow(bt, cmap='GnBu', interpolation='nearest', vmax = -np.min(bt))
                ax[0,1].set_title('Normalized Bathymetry')
