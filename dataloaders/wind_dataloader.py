    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith('.npy'): 
                ...
                tensor_layer_2 = torch.tensor(data[:, :, 2], dtype=torch.float32)  
                tensor_layer_3 = torch.tensor(data[:, :, 3], dtype=torch.float32)  
                ...
                if augmentation == True : 
                    ...
                    for i in range(4) :
                        ...
                        final_tensors.append((tensor_layer_1, tensor_layer_2, tensor_layer_3, tensor_layer_target))
...
class ODBADataset(Dataset):
    ...
    def __getitem__(self, idx):        
        traj, u_wind, v_wind, target_data = self.data[idx]
        input_data = torch.cat((traj, u_wind, v_wind), dim=-1)
