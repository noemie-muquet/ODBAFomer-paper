    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith('.npy'): 
                ...
                tensor_layer_2 = torch.tensor(data[:, :, 1], dtype=torch.float32)  
                tensor_layer_3 = torch.tensor(data[:, :, 2], dtype=torch.float32)  
                tensor_layer_4 = torch.tensor(data[:, :, 3], dtype=torch.float32)  
                tensor_layer_target = torch.tensor(data[:, :, -1], dtype=torch.float32)
            
                if augmentation == True : 
                  
                    tensor_layer_1 = tensor_layer_1.unsqueeze(0)
                    tensor_layer_2 = tensor_layer_2.unsqueeze(0)
                    tensor_layer_3 = tensor_layer_3.unsqueeze(0)
                    tensor_layer_4 = tensor_layer_4.unsqueeze(0)
                    tensor_layer_target = tensor_layer_target.unsqueeze(0)
            
                    for i in range(4) :
                    
                        TF.rotate(tensor_layer_1, angle = i*90)
                        TF.rotate(tensor_layer_2, angle = i*90)
                        TF.rotate(tensor_layer_3, angle = i*90)
                        TF.rotate(tensor_layer_4, angle = i*90)
                        TF.rotate(tensor_layer_target, angle = i*90)
                        
                        tensor_layer_1 = tensor_layer_1.unsqueeze(-1)
                        tensor_layer_2 = tensor_layer_2.unsqueeze(-1)
                        tensor_layer_3 = tensor_layer_3.unsqueeze(-1)
                        tensor_layer_4 = tensor_layer_4.unsqueeze(-1)
                        tensor_layer_target = tensor_layer_target.unsqueeze(-1)
                        
                        final_tensors.append((tensor_layer_1, tensor_layer_2, tensor_layer_3, tensor_layer_4, tensor_layer_target))
                        
                        tensor_layer_1 = tensor_layer_1.squeeze(-1)
                        tensor_layer_2 = tensor_layer_2.squeeze(-1)
                        tensor_layer_3 = tensor_layer_3.squeeze(-1)
                        tensor_layer_4 = tensor_layer_4.squeeze(-1)
                        tensor_layer_target = tensor_layer_target.squeeze(-1)
                    
                    TF.hflip(tensor_layer_1)
                    TF.hflip(tensor_layer_2)
                    TF.hflip(tensor_layer_3)
                    TF.hflip(tensor_layer_4)
...
class ODBADataset(Dataset):
    ...
    def __getitem__(self, idx):        
        traj, u_wind, v_wind, bathy, target_data = self.data[idx]
        input_data = torch.cat((traj, u_wind, v_wind, bathy), dim=-1)
